  ...
  methods: {
    async getList() {
      try {
        ...
        const res = await apiHandoverTags.getHandoverTagsList(this.getPayLoad)
        this.tbData = res
      } catch {
        // pass
      } finally {
    ...
    async sequenceChange({ oldIndex, newIndex }) {
      ...
      try {
        await apiHandoverTags.updateHandoverTagsSequence(params, id)
    ...
    handleAction({ action, row = null }) {
      if (action !== 'delete') {
        ...
        const id = row.id
        const deleteApi = apiHandoverTags.deleteHandoverTags
