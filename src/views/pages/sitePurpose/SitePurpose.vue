<template lang="pug">
  #sitePurpose.views.emsTheme(ref="view")
...
export default {
  ...
  methods: {
    async getList() {
      ...
      try {
        const res = await apiSitePurpose.getSitePurposeList(this.getPayLoad)
        this.tbData = res
      } catch {
        // pass
      } finally {
    ...
    handleAction({ action, row = null }) {
      if (action !== 'delete') {
        ...
        const id = row.id
        const deleteApi = apiSitePurpose.deleteSitePurpose
