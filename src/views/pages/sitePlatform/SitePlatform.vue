<template lang="pug">
  #sitePlatform.views.emsTheme(ref="view")
...
export default {
  ...
  methods: {
    async getList() {
      ...
      try {
        const res = await apiSitePlatform.getSitePlatformList(this.getPayLoad)
        this.tbData = res
      } catch {
        // pass
      } finally {
    ...
    handleAction({ action, row = null }) {
      if (action !== 'delete') {
        ...
        const id = row.id
        const deleteApi = apiSitePlatform.deleteSitePlatform
