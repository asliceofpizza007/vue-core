<template lang="pug">
  #siteDynamicRequired.views.emsTheme(ref="view")
...
export default {
  ...
  methods: {
    async getList() {
      ...
      try {
        const res = await apiSiteDynamic.getSiteDynamicRequiredList(this.getPayLoad)
        this.tbData = res
      } catch {
        // pass
      } finally {
    ...
    handleAction({ action, row = null }) {
      if (action !== 'delete') {
        ...
        const id = row.id
        const deleteApi = apiSiteDynamic.deleteSiteDynamicRequired
