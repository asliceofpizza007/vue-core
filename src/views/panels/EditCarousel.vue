...
export default {
  ...
  watch: {
    'formData.link': function (value) {
      if (!value) { this.formData.openType = null }
    },
  },
  created() {
    if (this.id) {
  ...
  methods: {
    async getRowData() {
      this.loading = true
      try {
        const res = await apiCarousel.getSlideBanner(this.id)
    ...
    submitWhenValidate: debounce(async function () {
      ...
      let api = apiCarousel.addSlideBanner
      let message = '資料新增成功'
      if (this.action === 'edit') {
        postData.push(this.id)
        api = apiCarousel.updateSlideBanner
        message = '資料更新成功'
      }
      try {
        this.loading = true
