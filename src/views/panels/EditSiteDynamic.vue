...
export default {
  ...
  methods: {
    async getRowData() {
      this.loading = true
      try {
        const res = await apiSiteDynamic.getSiteDynamicRequired(this.id)
    ...
    submitWhenValidate: debounce(async function () {
      ...
      let api = apiSiteDynamic.addSiteDynamicRequiredList
      let message = '資料新增成功'
      if (this.action === 'edit') {
        postData.push(this.id)
        api = apiSiteDynamic.updateSiteDynamicRequired
        message = '資料更新成功'
      }
      try {
        this.loading = true
